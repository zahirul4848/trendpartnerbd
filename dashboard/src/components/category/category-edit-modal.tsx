...
import { useGetCategoryQuery, useUpdateCategoryMutation } from '@/lib/features/category-api-slice';
...
const CategoryEditModal = ({setRequestRefetch, setShowEditModal, showEditModal, id}: TCategoryEdit) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async(data: TCategorySchema)=> {
      if(file) {
        ...
        try {
          ...
          if(uploadData) {
            const response = await updateCategory({id, data: {name: data.name, imageUrl: uploadData}});
        ...
        try {
          const response = await updateCategory({id, data: {name: data.name, imageUrl: ""}});
          toast.success(response.data.message);
          setRequestRefetch(true);
          setShowEditModal(false);
        } catch (error: any) {
            ...
            <div className="relative rounded-lg bg-gray-100 dark:bg-gray-800 shadow p-2 md:p-4">
              ...
              {category && (
                <form className="space-y-6 mt-4" onSubmit={handleSubmit}>
                  ...
                  <div>
                    <Image 
                      src={file ? URL.createObjectURL(file) : category.imageUrl.url}
                      className="w-[12rem] h-[10rem] my-4 object-cover" width={1024} height={1024} alt='Preview' 
                    />
                  </div>
                  <div>
                    <button 
